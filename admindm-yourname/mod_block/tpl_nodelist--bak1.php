...
<td width="60" >排序</td>
 <td width="260"  align="center">标题</td>
  ...
  <td align="center">预览 </td>
   <td width="200" align="center">分配给模板 <a href="http://www.demososo.com/detail-116.html" target="_blank">查看教程 ></a></td>
...
//if(getnum($sqleff)>0) {
 ...
 if(strpos($keff, 'o==')>0){ }
    ...
    $i++;
  if($i>12) $i=$i-12;
    $effbg=' class="effbg_'.$i.'"';
 //$effectlink = 'mod_effect.php?lang='.LANG.'&type=dh';
 $blockfile = EFFECTROOTADMIN.'block/'.$keff.'.php';
  $blockfilev='';
 if(!is_file($blockfile))  $blockfilev='<br /><span class="cred">'.$keff.'.php 文件不存在</span>';
...
echo '<tr><td colspan="8" class="sidebarsubtitle"></td></tr>';
...
<td align="center"><input type="text" name="<?php echo $tidmain;?>"  value="<?php echo $vcat['pos'];?>" size="5" /></td>
 <td  <?php echo $effbg;?>>  <?php   echo $name;?></td>
...
<?php 
  ...
  else {// echo '<tr><td colspan="8" align="center">暂无内容</td></tr>';
        }
