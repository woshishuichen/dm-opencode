...
// }
   ...
   if(substr($abc4,0,3)<>'nd_') 
    ...
    if ($abc1 == '')   $abc1 = '请输入标题';  
      ...
      $ss = "insert into ".TABLE_BLOCK." (pidname,pbh,type,pid,name,typeadmin,effect,maxline,dhtrigger,cssname,more,kv,lang,dateedit) values ('$pidname','$user2510','nd','$abc2','$abc1','$abc3','$abc4','$abc5','$abc6','$abc7','$abc8','$return_v','".LANG."','$dateall')";
      //echo $ss;exit;
      iquery($ss);
      alert("添加成功");
      jump($jumpv);
 ...
 $ss = "update ".TABLE_BLOCK." set name='$abc1',pid='$abc2',typeadmin='$abc3',effect='$abc4',maxline='$abc5',dhtrigger='$abc6',cssname='$abc7',more='$abc8',dhpara='$abc9'$kv_v,dateedit='$dateall' where pidname='$pidname' $andlangbh limit 1";
      iquery($ss);  
       // echo $ss;exit;
        jump($jumpvpf.'&act=edit');   
 }
...
$row22 = getrow($sql);
 if($row22=='no'){echo '出错，请检查当前内容和模板是否统一';exit;}
$effect = $row22['effect']; $typeadmin = $row22['typeadmin']; 
...
if($act=='edit'){ 
 ...
 ?>
     ...
     <tr>
      <td width="12%" class="tr">效果的类型：</td>
      <td width="88%"> 
     <select name="typeadmin" class="">
...
<span class="cgray">此选项仅为后台管理用。</span>
        </td>
    </tr>
...
<option  value="">请选择</option> 
 ...
 <?php 
 select_from_arr($arr_blocknd,$effect,'');
       
//                   $sql = "SELECT * from ".TABLE_BLOCK."  where type='effect_nd' and stylebh= '$stylebh' and sta_visible='y'  order by pos desc,id desc";
