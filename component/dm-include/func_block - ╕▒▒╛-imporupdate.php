...
//2013/***/.jpg or .swf
  $pidstring = substr($pidname,0,4);
   ...
   elseif($pidstring=='ndli') {
	...
	elseif($pidstring=='bloc') { //echo 'cancel region sub block tiaoyong...';
...
function region($region){
 Global $rgindex_design;
 //echo $regionindex_design; 

Global $andlangbh;Global $curstyle;

$regsta_sub = get_field(TABLE_REGION,'sta_sub',$region,'pidname');
...
Global $andlangbh;
	 $sqlbuju = "SELECT * from ".TABLE_REGION." where pidname='$v' $andlangbh order by pos desc,id limit 1";
	// echo $sqlbuju;
	 if(getnum($sqlbuju)>0){
...
Global $andlangbh;
$sql2 = "SELECT cssname,sta_allwidth_cnt from ".TABLE_REGION." where pidname='$region'  $andlangbh order by id limit 1";
 
if(getnum($sql2)>0){
	$row2 = getrow($sql2); 
	$cssname =  $row2['cssname'];
	$sta_allwidth_cnt = $row2['sta_allwidth_cnt'];
	$cssnamev = $cssname<>''?' class="'.$cssname.'"':'';
echo '<div '.$cssnamev.'>';
if($sta_allwidth_cnt=='n') echo '<div class="container">';
//-----------------------
	 ...
	 if(getnum($sqlbuju)>0){
		 ...
		 // pre($result);	
			...
			if($desptext<>'') $despv = $desptext;
				...
				echo '<div class="'.$cssname.'">';
				if($sta_name=='y') echo '<h3>'.$name.'</h3>';
				 if($blockid==''){ echo $despv;}
				 else {block($blockid);}
				echo '</div>'; 
...
if(dmlogin()){ //is in func_block. bec declare.bec this file repeat require
	echo 'aaaa';
	 	...
	 	//echo '</div>';	
if($sta_allwidth_cnt=='n') echo '</div>';	
...
function regionblockecho($v){ 
	// pre($v);
	        $tid=$v['id'];$pid=$v['pid'];$pidname=$v['pidname'];
            $name=$v['name'];
            $cssname=$v['cssname'];$cssstyle=$v['cssstyle'];  
			$wrap=$v['wrap'];
			//$style=$v['style'];
			$blockid=$v['blockid'];
			$effect=$v['effect'];$despjj=$v['despjj'];
			$bgcolor=$v['bgcolor'];$bgimg=$v['bgimg'];$bgeffect=$v['bgeffect'];
			
			$linkurl=$v['linkurl'];//如果有值，则会出现 更多链接
			$linktitle=$v['linktitle'];$linkcss=$v['linkcss'];$linkposi=$v['linkposi'];
			$titlebox=$v['titlebox'];$titleboxcssname=$v['titleboxcssname'];
			$titlestyle=$v['titlestyle'];$titleline=$v['titleline'];$titlelinewrap=$v['titlelinewrap'];
			$titleimg=$v['titleimg'];$titlestylesub=$v['titlestylesub'];

			$sta_allwidth_box=$v['sta_allwidth_box'];$sta_allwidth_cnt=$v['sta_allwidth_cnt'];
			...
			$classv='';
			$allwidth_box = $sta_allwidth_box=='y'?' ':' container';
			//$allwidth_cnt = $sta_allwidth_cnt=='y'?'':'container';
			$allwidth_cntfirst = $sta_allwidth_cnt=='y'?'':'<div class="container">';
			$allwidth_cntlast = $sta_allwidth_cnt=='y'?'':'</div>';

             $classv= " class='block $cssname $allwidth_box'";   //use normal type               
            // else $classv= "";
         
           
             //--title and titlecenter--------------------------
			...
			if($bgeffect=='para'){ 
				$sta_bgparav = ' data-type=imgpara';
			}

			$stylev  ='';
			if($bgcolor<>'' || $bgimg<>'' || $cssstyle<>'') $stylev =' style="background:'.$bgcolor.$bgimgv.$bgsize.';'.$cssstyle.'"';

             //-end title and titlecenter----------------------

              $classv2 = $cssname;//use clear and wrap type

            // if($style<>'') $stylev= " style=$style";
            // else $stylev= "";
 
if($wrap=='clear')  echo    "<div id=clearblock$tid class='clearblock $classv2'>  </div>"; 
else if($wrap=='divbegin') echo    "<div id=wrapblock$tid  class='wrapblock $classv2'>";
else if($wrap=='divend') echo    "</div>";
else{	
	//only wrap is normal:
		   if(dmlogin()){ //is in func_block. bec declare.bec this file repeat require
      ...
      //make simple,no use edit in sub region.------  
		...
		//	if(dmlogin()){ //is in func_block. bec declare.bec this file repeat require
			...
			//    echo  '<a href="" class="dmedit dmeditregion">编辑</a>';
   ...
   //mainregion
 	...
 	if($type=='regionblock'){  
      ...
      if($typefirst=='regionindex') $typereg='index';
 		  ...
 		  echo  '<div style="position:relative"><a target="_blank" href="'.$linkv.'" class="dmedit dmeditregion">编辑区域</a></div>';
 ...
 $linkv = $urlhere.'/mod_region/mod_regcommon.php?lang='.LANG.'&type=common&pidname='.$pidname.'&file=subaddedit&act=edit&tid='.$tid;
 	...
 	if($type=='vblock'){
 		...
 		$linkv = $urlhere.'/mod_block/mod_block.php?lang='.LANG.'&file=edit&act=edit&tid='.$tid;
 		  ...
 		  echo  '<a target="_blank" href="'.$linkv.'" class="dmedit">编辑区块</a>';
       ...
       $tid =  get_field(TABLE_IMGFJ,'id',$pidname,'kv');
 		...
 		$linkv = $urlhere.'/mod_imgfj/mod_imgfj.php?lang='.LANG.'&pid=name&file=edit&act=edit&tid='.$tid;
 		 echo  '<a target="_blank" href="'.$linkv.'" class="dmedit">编辑附件</a>';
	 ...
	 if($type=='ndli'){
 		...
 		$linkv = $urlhere.'/mod_block/mod_nodelist.php?lang='.LANG.'&pidname='.$pidname.'&file=addedit&act=edit';
 		 echo  '<a target="_blank" href="'.$linkv.'" class="dmedit">编辑文章列表</a>';
	  ...
	  if($type=='group'){
	   ...
	   if($type=='prog'){
         echo  '<a target="_blank" href="javascript:void(0)" style="cursor:default" class="dmedit">请编辑'.$pidname.'</a>';
