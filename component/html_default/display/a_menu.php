...
<?php //get menu
 ...
 if($sta_cusmenu=='y')  {
    if($cusmenu=='') echo 'no text in menu.pls edit.';
 	else echo web_despdecode($cusmenu);}
else{

echo '<ul class="m">';
//----------------------
global $andlangbh;
$sqlmenu = "SELECT * from ".TABLE_MENU." where pid='0' and stylebh = '$curstyle' and sta_visible='y' $andlangbh order by pos desc,id";	
if(getnum($sqlmenu)>0){
//------------
		...
		foreach($menulist as $v){	
            ...
            if($type=='page'){
				 ...
				 if($pagearr=='no'){$name='单页面不存在';$linkurl='0';}
                    ...
                    echo '<li class="m"><a class="m'.$activemenu.'" '.linktarget($linkurl).' href="'.$linkurl.'">'.$name.'</a>';
                    //-----sub menu----------------
                    if($menu_xiala<>'') echo decode($menu_xiala);
                    else{
						$sql22 = "SELECT * from ".TABLE_MENU." where pid='$pidname' and stylebh = '$curstyle' and type<>'cate' and sta_visible='y' $andlangbh order by pos desc,id";	
						if(getnum($sql22)>0){
									...
									if($type2=='page'){
										...
										if($pagearr2=='no' ){
											$name2='单页面不存在';$linkurl2='0';
												...
												$tid2 = $pagearr2['id'];
												$name2 = $pagearr2['name'];
						 						$alias_jump2=$pagearr2['alias_jump'];	
						 						$aliascc2 = alias($pidname2,'page');
						 						$linkurl2 = url('page',$aliascc2,$tid2,$alias_jump2); 
						 					}
			 						}	
					                echo '<li><a '.linktarget($linkurl2).' href="'.$linkurl2.'">'.$name2.'</a></li>';
