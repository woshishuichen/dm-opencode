...
function zb_insert($array){ //转变函数
	...
	$v  = gpc_add($v);
        ...
        $abc='abc'.$i;
        global $$abc;
        $$abc=htmlspecialchars($v);//---for security script zhuru--htmlentities will let chinese lunma
...
if($css<>''){
   ...
   if(ADMIN=='y') $target='_blank';
           ...
           if(strpos($link,'://')>1) $target='_blank';
          else $target=LINKTARGET_F;
  }
...
if($admin=='admin') $link = l($userurl.$url,$url.$jumpvtext,'','');
...
function  linktarget($v){
       if(strpos($v,'://')>1) return $target=' target="_blank"';
          else return '';
}
//------------
